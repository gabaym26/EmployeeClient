...
export default function AddWorker(data) {
    ...
    const [rolesEmployee, setRolesEmployee] = useState([{ RoleId: 0, IsAdministrative: '', StartDate: Date.now }]);
    const [errorMessageEarlyDate, setErrorMessageEarlyDate] = React.useState('');
    ...
    const handleRoleChange = (index, field, value) => {
        setRolesEmployee(prevRoles => {
            return prevRoles.map((role, i) => {
                if (i === index) {
                    let available = roles.filter(
                        (role) => !rolesEmployee.some((selectedRole) => selectedRole.RoleId === role.id)
                    );
                    if (rolesEmployee[index]?.RoleId != 0) {
                        const roleToDelete = rolesEmployee[index];
                        available = [...available, roles.find(role => role.id === roleToDelete.RoleId)]
                    }
                    available = available.filter(availableRole => availableRole.id !== value);
                    setAvailableRoles(available);
                ...
                {rolesEmployee.map((role, index) => (
                    <div key={index}>
                        ...
                        <Select fullWidth
                            ...
                            value={role ? role.IsAdministrative : 0}
                        ...
                        <TextField
                            ...
                            fullWidth
                            type="date"
                            value={dayjs(role ? role.StartDate : '').format('YYYY-MM-DD')}
                            onChange={(e) => {
